...
class SearchDoctorController extends Controller
{
    ...
    public function search_doctor(Request $request)
    {
        ...
        $faskes_kode = $request->input('faskes_kode');
        $gelar_depan = $request->input('gelar_depan');

        if($gelar_depan=="dr"&&$nama==""&&$jadwal_hari==""&&$jenis_kelamin==""&&$spesialisasi=="")
        {
            $data = DB::collection('dokter')
            ->when($gelar_depan, function ($query, $gelar_depan) {
                return $query->where('gelar_depan', $gelar_depan);
            })
          ->where('gelar_blk',null)
          ->orderBy('nama','asc')
          ->paginate(10);
     ...
     ->when($jadwal_hari, function ($query, $jadwal_hari) {
         return $query->where('jadwal.jadwal.hari', $jadwal_hari);
     })
     ->when($jadwal_hari, function ($query, $jadwal_hari) {
        return $query->whereNotIn('jadwal.jadwal.mulai',['00:00']);
    })
    ->when($jadwal_hari, function ($query, $jadwal_hari) {
        return $query->whereNotIn('jadwal.jadwal.selesai',['00:00']);
    })
    ...
    ->when($spesialisasi, function ($query, $spesialisasi) {
      ...
      ->when($faskes_kode, function ($query, $faskes_kode) {
        return $query->where('jadwal.faskes_kode', $faskes_kode);
      })
      ->orderBy('nama','asc')
      ->paginate(10);
    ...
    public function suggestion(Request $request)
    {
        ...
        $listDoctor = Doctor::select('gelar_depan','gelar_blk','nama','spesialis')->where('nama', 'like', '%'.$name.'%')->limit($limit)->get();
        $listSpesialis = DB::collection('spesialis_dokter')->select('gelar_depan','gelar_blk','nama','spesialis')->where('nama', 'like', '%'.$name.'%')->limit($limit)->get();

        return response()->json([
            	'status'  => 200,
                'ListDoctor'    => $listDoctor,
                'ListSpesialis' => $listSpesialis,
            	'message' => 'Successful',
        ]);
